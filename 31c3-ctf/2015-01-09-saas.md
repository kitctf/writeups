...
We notice several functions with the prefix `unguarded_`. One of them is
`unguarded_linear_insert`. It underflows the buffer, which is not helpful to us
...
[The C++ we created to record the
