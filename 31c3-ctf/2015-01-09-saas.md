...
![comparator]({{ site.url }}/imgs/31c3ctf_saas_1.png)
...
The C++ code assumes that the function `f` has exactly two parameters. It
...
For further analysis and to make use of the internals, we [rewrote the relevant
functions in a more legible and less general
manner](https://github.com/kitctf/writeups/blob/master/31c3-ctf/saas/mysort.h).

The source code is already commented to explain what each part does and where
the relevant parts are. In particular, we will make use of the following
function:
...
It is used as a subroutine of quicksort, to move the pivot element to its
correct position. As you can see, we can control `first` because we can 
arbitrarily increment it. The swap will not happen because we return before that. 
The calling function is the quicksort routine:
...
What happens if we make the call to `unguarded_partition_pivot` return an
iterator past the end of the buffer? The recursion into the "right half" `[cut, last)` will do nothing because `cut > last`. Then the algorithm goes on to sort 
the "left half" `[first, cut)`, which is bigger then the original buffer! From here
on, if we switch to a standard comparator that just compares the elements, the
algorithm will happily sort our "extended" buffer without complaining.
...
input plus 1024 extra words that happen to lie on the stack).

So we should be able to both read and write arbitrarily even beyond the end of 
our buffer!

From here we just took a very simple approach: We recorded the answers
that a simple less-than comparator would give to `std::sort` when sorting the
array

    {m, 1, 2, ..., m - 1, 0}

The permutation that `std::sort` will do on this array is basically to swap the
first and last element. And if we feed it the answers that we recorded, it will
happily repeat this exact permutation, regardless of the actual values in the
array. [We created a program](https://github.com/kitctf/writeups/blob/master/31c3-ctf/saas/compute_answers.cpp) that will create a sequence of answers
that swaps the elements at offsets `0` and `offset`, when `std::sort` is called
with size `n`.

If we now define a comparator in SaaS that returns this exact sequence of
answers, we will be able to swap a value that we control with any other value
on the stack.

## Leak a libc address

We can read an arbitrary value on the stack, we just need to pick one! A good
...
One thing to note is that we need to do the writes in decreasing order because
the sort might permute the elements between the first and last one. The [final
exploit code](https://github.com/kitctf/writeups/blob/master/31c3-ctf/saas/exploit.py) reliably gives shell access and we are able to retrieve the flag

    $ python2 exploit.py
    [*] leaked libc base = 0xf745a000
    [*] writing ROP 15 0xf7604ec0
    [*] writing ROP 14 0x0
    [*] writing ROP 13 0xf7498770
    [*] writing ROP 12 0x400
    [*] writing ROP 11 0xf7604ec0
    [*] writing ROP 10 0x0
    [*] writing ROP 9 0xf7498760
    [*] writing ROP 8 0xf7535160
    [*] Enjoy your shell ;)
